...
// The map below maps a msg.message.envelope.room to actual room without doing
// an admin-required api lookup each time.  To update this list, run the
// following, using the hipchat_deploy_token from webapp's secrets.py.
//
...
//
// If culture cow should not be listening in any of the newly added rooms, add
// it to HUBOT_HIPCHAT_ROOMS_BLACKLIST in bin/secrets. You'll need to add the
// jabber ID of the room to the blacklist, which you can find in
// https://khanacademy.hipchat.com/account/xmpp.
var ROOMS = {
    "*schwag*": "*Schwag*",
    "1762": "1762",
    "1s_and_0s": "1s and 0s",
    ...
    robot.setTopic = function(room, topic) {
        ...
        this.http("https://api.hipchat.com/v1/rooms/topic")
            ...
            .post(params, function(err, res, body) {
                // This API call appears, as of 8/8/15, to always return an
                // empty 200 no matter what you give it, so there's no point
                // in trying to do error handling here.
            });
    };
